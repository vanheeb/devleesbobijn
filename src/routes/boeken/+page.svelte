<script lang="ts">
	...
	async function handleSubmit() {
		if (!selectedPackageId || !selectedDate || !customerName || !customerEmail || !customerPhone) {
			error = 'Vul alle verplichte velden in.'; return;
		}
		if (!acceptedTerms) { error = 'Je moet de huurvoorwaarden aanvaarden.'; return; }
...
<div class="max-w-4xl mx-auto px-4 py-12">
	<div class="text-center mb-8">
		<h1 class="text-3xl md:text-4xl font-display mb-4">Boek jouw vleesbobijn</h1>
		<p class="text-gray-600">Volg de stappen hieronder om je boeking af te ronden.</p>
	</div>

	<!-- Voortgangsbalk -->
	<div class="flex items-center justify-center gap-2 mb-10">
		{#each [{ n: 1, label: 'Pakket' }, { n: 2, label: 'Datum' }, { n: 3, label: "Extra's" }, { n: 4, label: 'Gegevens' }] as s}
			<div class="flex items-center gap-2">
				<div class="w-8 h-8 rounded-full flex items-center justify-center text-sm font-display transition-colors
					{step > s.n ? 'bg-secondary text-primary-dark' : step === s.n ? 'bg-primary text-white' : 'bg-gray-200 text-gray-500'}">
					{step > s.n ? '✓' : s.n}
				</div>
				<span class="text-sm font-medium hidden sm:inline {step >= s.n ? 'text-primary' : 'text-gray-400'}">{s.label}</span>
			</div>
			{#if s.n < 4}<div class="w-8 h-0.5 {step > s.n ? 'bg-secondary' : 'bg-gray-200'}"></div>{/if}
		{/each}
	</div>
	...
	{#if step === 1}
		<div>
			<h2 class="text-xl font-display mb-6">Kies je pakket</h2>
			<p class="text-sm text-gray-500 mb-3">🐔 Kip</p>
	...
	{#if step === 2}
		<div>
			<div class="flex items-center justify-between mb-6">
				<h2 class="text-xl font-display">Kies je datum</h2>
				<button onclick={() => (step = 1)} class="text-sm text-primary hover:underline">← Terug naar pakketten</button>
			</div>
			{#if selectedPackage}
				<div class="bg-primary/5 rounded-xl p-4 mb-6 flex justify-between items-center">
					...
					<span class="font-display text-primary">{formatPriceShort(selectedPackage.price)}</span>
				</div>
			{/if}
	...
	{#if step === 3}
		<div>
			<div class="flex items-center justify-between mb-6">
				<h2 class="text-xl font-display">Extra's toevoegen</h2>
				<button onclick={() => (step = 2)} class="text-sm text-primary hover:underline">← Terug naar datum</button>
			</div>
			<div class="bg-primary/5 rounded-xl p-4 mb-6">
				<div class="flex justify-between items-center">
					<span class="font-medium">{selectedPackage?.name}</span>
					<span class="font-display text-primary">{formatPriceShort(selectedPackage?.price || 0)}</span>
				</div>
			...
			{#if extrasTotal > 0}
				<div class="bg-secondary/10 rounded-xl p-4 mb-6 flex justify-between">
					...
					<span class="font-display">{formatPriceShort(extrasTotal)}</span>
				</div>
			{/if}
			<button onclick={goToStep4} class="w-full bg-primary hover:bg-primary-dark text-white font-display py-4 rounded-xl text-lg transition-colors">
	...
	{#if step === 4}
		<div>
			<div class="flex items-center justify-between mb-6">
				<h2 class="text-xl font-display">Jouw gegevens</h2>
				<button onclick={() => (step = 3)} class="text-sm text-primary hover:underline">← Terug naar extra's</button>
			</div>
			<div class="grid grid-cols-1 md:grid-cols-2 gap-8">
				<div class="space-y-4">
					{#each [
						{ id: 'name', label: 'Naam *', type: 'text', bind: customerName, req: true },
					] as _f}<!-- structured below -->{/each}
					<div>
						<label for="name" class="block text-sm font-medium mb-1">Naam *</label>
						<input type="text" id="name" bind:value={customerName} required class="w-full px-4 py-3 rounded-xl border border-gray-300 focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none" />
					</div>
					<div>
						<label for="email" class="block text-sm font-medium mb-1">E-mail *</label>
						<input type="email" id="email" bind:value={customerEmail} required class="w-full px-4 py-3 rounded-xl border border-gray-300 focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none" />
					</div>
					<div>
						<label for="phone" class="block text-sm font-medium mb-1">Telefoon *</label>
						<input type="tel" id="phone" bind:value={customerPhone} required class="w-full px-4 py-3 rounded-xl border border-gray-300 focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none" />
					</div>
					<div>
						<label for="address" class="block text-sm font-medium mb-1">Adres</label>
						<input type="text" id="address" bind:value={customerAddress} class="w-full px-4 py-3 rounded-xl border border-gray-300 focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none" />
					</div>
					<div>
						<label for="city" class="block text-sm font-medium mb-1">Gemeente</label>
						<input type="text" id="city" bind:value={customerCity} class="w-full px-4 py-3 rounded-xl border border-gray-300 focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none" />
					</div>
					<div>
						<label for="eventType" class="block text-sm font-medium mb-1">Soort feest</label>
						<select id="eventType" bind:value={eventType} class="w-full px-4 py-3 rounded-xl border border-gray-300 focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none">
					...
					<div>
						...
						<input type="number" id="guests" bind:value={guestCount} min="1" class="w-full px-4 py-3 rounded-xl border border-gray-300 focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none" />
					</div>
					<div>
						<label for="notes" class="block text-sm font-medium mb-1">Opmerkingen</label>
						<textarea id="notes" bind:value={notes} rows="3" class="w-full px-4 py-3 rounded-xl border border-gray-300 focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none resize-y"></textarea>
					</div>
				</div>

				<div>
					<div class="bg-white rounded-2xl border p-6 sticky top-20">
						<h3 class="font-display text-lg mb-4">Overzicht</h3>
						<div class="space-y-3 mb-6">
							<div class="flex justify-between">
								<span>{selectedPackage?.name}</span>
								<span class="font-display">{formatPriceShort(selectedPackage?.price || 0)}</span>
							</div>
						...
						<div class="mt-5 space-y-3 text-sm">
							<label class="flex items-start gap-2 cursor-pointer">
								<input type="checkbox" bind:checked={acceptedTerms} class="mt-1 w-4 h-4 accent-primary" />
								<span class="text-gray-700">Ik aanvaard de <a href="/voorwaarden" target="_blank" rel="noopener" class="text-primary underline">huurvoorwaarden</a> (€250 waarborg, €100 reinigingskost bij niet-propere teruggave).</span>
							</label>
							<label class="flex items-start gap-2 cursor-pointer">
								<input type="checkbox" bind:checked={acceptedPrivacy} class="mt-1 w-4 h-4 accent-primary" />
								<span class="text-gray-700">Ik ga akkoord met de <a href="/privacy" target="_blank" rel="noopener" class="text-primary underline">privacyverklaring</a>.</span>
							</label>
						</div>
						{#if error}
							<div class="bg-red-50 border border-red-200 rounded-xl p-3 mt-4 text-red-700 text-sm">{error}</div>
						{/if}
