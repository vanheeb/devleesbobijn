<script lang="ts">
	const faqs = [
		...
		{ q: 'Kan ik mijn boeking annuleren?', a: 'Neem contact met ons op via telefoon of email om een annulering te bespreken. We zoeken altijd naar een passende oplossing.' }
	];
...
<svelte:head>
	...
	<meta name="description" content="Antwoorden op veelgestelde vragen over de vleesbobijn: pakketten, prijzen, waarborg, levering en annulering." />
	<meta property="og:title" content="Veelgestelde vragen - de vleesbobijn" />
	<meta property="og:description" content="Antwoorden op veelgestelde vragen over de vleesbobijn: pakketten, prijzen, waarborg, levering en annulering." />
...
<div class="max-w-3xl mx-auto px-4 py-12">
	<div class="text-center mb-12">
		<h1 class="text-3xl md:text-4xl font-display mb-4">Veelgestelde vragen</h1>
		...
		{#each faqs as faq, i}
			<div class="bg-white rounded-xl border border-gray-200 overflow-hidden">
				...
				>
					<span>{faq.q}</span>
					<svg
						class="w-5 h-5 text-primary transition-transform flex-shrink-0 ml-4 {openIndex === i ? 'rotate-180' : ''}"
	...
	<div class="mt-12 text-center bg-primary/5 rounded-2xl p-8">
		<h2 class="text-xl font-display mb-2">Nog vragen?</h2>
		<p class="text-gray-600 mb-4">Neem gerust contact met ons op!</p>
		<div class="flex flex-col sm:flex-row gap-4 justify-center">
			<a href="tel:+32479392283"
				class="inline-flex items-center justify-center gap-2 bg-primary text-white font-display px-6 py-3 rounded-xl hover:bg-primary-dark transition-colors">
				📞 +32 479 39 22 83
			</a>
			<a href="/contact"
				class="inline-flex items-center justify-center gap-2 bg-white text-primary border border-primary font-display px-6 py-3 rounded-xl hover:bg-primary/5 transition-colors">
				✉️ Contactformulier
			</a>
		</div>
	</div>
